...
4. **JSON attributes** — All user-editable JSON strings use `jsontypes.Normalized` via helpers in `internal/provider/jsonschema.go` (semantic equality for plan). Still canonicalize API payloads with `client.CanonicalJSON` when building state from reads. Do not use a plan modifier alone for JSON. For notification channels, `config` is receiver-only (one `*_configs` array); set display name with resource `name`, not inside `config` (`client.NormalizeChannelConfigJSON` strips `name` on read/plan).
